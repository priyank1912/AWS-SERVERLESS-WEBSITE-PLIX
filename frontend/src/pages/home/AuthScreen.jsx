	...
	const handleFormSubmit = async (e) => {
		...
		try {
			...
			const response = await fetch(
				`https://lqze31k6fk.execute-api.us-east-1.amazonaws.com/getUserByEmail?email=${encodeURIComponent(email)}`
			);
			...
			if (response.ok) {
				...
				if (data.user) {
					// User exists, redirect to login with email as query parameter
					navigate(`/login?email=${encodeURIComponent(email)}`);
				} else {
					// User does not exist, redirect to signup with pre-filled email
					navigate(`/signup?email=${encodeURIComponent(email)}`);
				}
			} else {
				// Handle the case where the user is not found (404 response)
	...
	return (
		<div className='hero-bg relative'>
			...
			<header className="max-w-6xl mx-auto flex items-center justify-between p-4 pb-10">
				<img src="/plix-logo.png" alt="Netflix Logo" className="w-32 md:w-52" />
